...
function openModal(data) {
    ...
    if (techWrap) {
        techWrap.innerHTML = '';
        const techs = Array.isArray(data.tech) ? data.tech : (data.tech ? [data.tech] : []);
        techs.forEach((t) => {
            if (!t) return;
...
triggers.forEach(trigger => {
    trigger.addEventListener('click', () => {
        const data = {
            ...
            tech: trigger.dataset.tech ? trigger.dataset.tech.split(',').map(s => s.trim()).filter(Boolean) : []
        };
        openModal(data);
    });
});
...
// === EMAILJS INIT ===
emailjs.init('9Pqq5KXCM3wuVy14z');
...
contactForm.addEventListener('submit', (e) => {
    ...
    emailjs.send('service_9ryq9ui', 'template_b0qcybv', {
        from_name: name,
        from_email: email,
        message: message,
        reply_to: email,
